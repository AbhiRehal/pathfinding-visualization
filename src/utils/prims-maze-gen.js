export function primsMazeGen() {
  ...
  for (const dir of directions) {
    if (inBounds(starterNode.x + dir.dx, starterNode.y + dir.dy, x_dir, y_dir)) {
      frontier_set.push(grid[starterNode.y + dir.dy][starterNode.x + dir.dx]);
      grid[starterNode.y + dir.dy][starterNode.x + dir.dx].className = 'frontier';
    }
  }
  ...
  do {
    ...
    for (const dir of directions) {
      if (
        ...
        (grid[random_cell_from_fs.y + dir.dy][random_cell_from_fs.x + dir.dx].className == 'node' ||
          grid[random_cell_from_fs.y + dir.dy][random_cell_from_fs.x + dir.dx].className == 'startNode')
      ) {
        neighbour_set.push(grid[random_cell_from_fs.y + dir.dy][random_cell_from_fs.x + dir.dx]);
      } else if (
        inBounds(random_cell_from_fs.x + dir.dx, random_cell_from_fs.y + dir.dy, x_dir, y_dir) &&
        grid[random_cell_from_fs.y + dir.dy][random_cell_from_fs.x + dir.dx].className == 'wall'
      ) {
